...
**The in-worker compiler runs, with four accommodations.**
...
And the same init reads the CommonJS `__filename`/`__dirname` globals, which wrangler's esbuild leaves bare in a worker bundle while the pool's optimizer injects shims - the first deployed publish died on "__filename is not defined" after the local suite passed, so `wrangler.jsonc` now defines both to virtual paths, and the disagreement is recorded here because the pool cannot currently reproduce it.
